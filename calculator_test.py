...
# 1. 补全计算器（加减乘除）的测试⽤例
# 2. 使⽤参数化完成测试的⾃动⽣成
# 3. 在调⽤测试⽅法之前打印【开始计算】，在测试⽅法之后打印【计算结束】
# 注意：
# * 使⽤等价类，边界值，因果图等设计测试⽤例
# * 测试⽤例中添加断⾔，验证结果
# * 灵活使⽤setup(),teardown(),setup_class(),teardown_class()
import pytest
...
def test_multiplication(a,b,expected):
    print("\na*b的结果为：" + str(multiplication_function(a, b)),"与预期值比较结果为：",multiplication_function(a, b) == expected)
